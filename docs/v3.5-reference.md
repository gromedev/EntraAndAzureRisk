...
6. Using += with arrays in PowerShell is inefficient - it creates a new array and copies all elements each time (O(n²) complexity). Optimize by using [System.Collections.Generic.List[object]] which has efficient .Add() and .AddRange() methods instead.
