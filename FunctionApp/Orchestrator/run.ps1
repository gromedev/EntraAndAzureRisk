...
try {
    ...
    #region Step 2: Index in Cosmos DB with Delta Detection and Retry
    ...
    while ($retryCount -lt $maxRetries -and -not $indexSuccess) {
        ...
        if ($indexResult -and $indexResult.Success) {
            ...
            if ($retryCount -lt $maxRetries) {
                Write-Warning "IndexInCosmosDB failed (attempt $retryCount/$maxRetries). Retrying in 60s..."
                Start-DurableTimer -Duration (New-TimeSpan -Seconds 60)
            }
            else {
                Write-Error "IndexInCosmosDB failed after $maxRetries attempts"
