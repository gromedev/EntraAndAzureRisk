...
.DESCRIPTION
    ...
    Collects:
    - Function App metadata (kind, state, httpsOnly, clientCertEnabled)
    - System-assigned managed identity info
    - User-assigned managed identity info
    - Host names and default host name
    - Contains relationships from RGs to Function Apps
    - hasManagedIdentity relationships linking Function Apps to SPs

    All output goes to azureresources.jsonl with resourceType = "functionApp".
#>
...
try {
    ...
    #region 2. Collect Function Apps from each subscription
    foreach ($sub in $subscriptions) {
        $subscriptionId = $sub.subscriptionId
        Write-Verbose "Scanning subscription: $($sub.displayName) ($subscriptionId)"

        try {
            # Get all web sites and filter for function apps
            ...
            foreach ($site in $sitesResponse.value) {
                ...
                $kind = $site.kind ?? ""
                if ($kind -notmatch 'functionapp') { continue }
                ...
                # Create contains relationship from RG to Function App
                if ($rgId) {
                    $containsRel = @{
            ...
            Write-Warning "Failed to collect Function Apps for subscription $subscriptionId`: $_"
        }
    }
    #endregion
    ...
    Write-Verbose "Function App collection complete: $($stats.FunctionApps) apps, $($stats.ManagedIdentityLinks) managed identity links"
    ...
    return @{
        ...
        Summary = @{
            ...
            functionApps = $stats.FunctionApps
    ...
    Write-Error "Function App collection failed: $_"
    return @{
        Success = $false
